...
Elm.Main.make = function (_elm) {
   ...
   var Input = F2(function (a,b) {
      return {direction: a,delta: b};
   });
   var snakeTouchesItself = function (snake) {
      return A2($List.any,
      function (position) {
         return _U.eq(position,snake.position);
      },
      snake.previousPositions);
   };
   var snakeTouchesApple = F2(function (snake,apple) {
      return _U.eq(apple.position,
      $Maybe.Just(snake.position)) || A2($List.any,
      function (position) {
         return _U.eq(apple.position,$Maybe.Just(position));
      },
      snake.previousPositions);
   });
   var toPixels = function (value) {
      return A2($Basics._op["++"],$Basics.toString(value),"px");
   };
   var mapSize = 20;
   var cap = function (num) {
      return _U.cmp(num,0) < 0 ? mapSize + num : _U.cmp(num,
      mapSize) > -1 ? num - mapSize : num;
   };
   var capPosition = function (_p0) {
      var _p1 = _p0;
      return {x: cap(_p1.x),y: cap(_p1.y)};
   };
   var newApple = function (apple) {
      var _p2 = A2($Random.generate,
      A2($Random.$int,0,mapSize - 1),
      apple.seed);
      var xPosition = _p2._0;
      var seed$ = _p2._1;
      var _p3 = A2($Random.generate,
      A2($Random.$int,0,mapSize - 1),
      seed$);
      var yPosition = _p3._0;
      var seed$$ = _p3._1;
      return {seed: seed$$
             ,position: $Maybe.Just({x: xPosition,y: yPosition})};
   };
   var stepApple = F2(function (apple,snake) {
      return A2(snakeTouchesApple,
      snake,
      apple) ? newApple(apple) : apple;
   });
   var canvasSize = 600;
   var blockSize = $Basics.round(canvasSize / mapSize);
   var blockStyle = _U.list([{ctor: "_Tuple2"
                             ,_0: "width"
                             ,_1: toPixels(blockSize)}
                            ,{ctor: "_Tuple2",_0: "height",_1: toPixels(blockSize)}
                            ,{ctor: "_Tuple2",_0: "position",_1: "absolute"}]);
   var snakeStyle = function (position) {
      return $Html$Attributes.style($List.concat(_U.list([blockStyle
                                                         ,_U.list([{ctor: "_Tuple2"
                                                                   ,_0: "top"
                                                                   ,_1: toPixels(blockSize * position.y)}
                                                                  ,{ctor: "_Tuple2"
                                                                   ,_0: "left"
                                                                   ,_1: toPixels(blockSize * position.x)}
                                                                  ,{ctor: "_Tuple2"
                                                                   ,_0: "font-size"
                                                                   ,_1: toPixels(blockSize)}])])));
   };
   var appleStyle = function (position) {
      return $Html$Attributes.style($List.concat(_U.list([blockStyle
                                                         ,_U.list([{ctor: "_Tuple2"
                                                                   ,_0: "top"
                                                                   ,_1: toPixels(blockSize * position.y)}
   ...
   var containerStyle = $Html$Attributes.style(_U.list([{ctor: "_Tuple2"
                                                        ,_0: "width"
                                                        ,_1: toPixels(canvasSize)}
                                                       ,{ctor: "_Tuple2",_0: "height",_1: toPixels(canvasSize)}
                                                       ,{ctor: "_Tuple2",_0: "position",_1: "relative"}
                                                       ,{ctor: "_Tuple2",_0: "background",_1: "#ccc"}]));
   var view = function (_p4) {
      var _p5 = _p4;
      var _p7 = _p5.snake;
      var appleNode = function () {
         var _p6 = _p5.apple.position;
         if (_p6.ctor === "Just") {
               return A2($Html.div,
               _U.list([appleStyle(_p6._0)]),
               _U.list([$Html.text("🍔")]));
            } else {
               return A2($Html.div,_U.list([]),_U.list([]));
            }
      }();
      var wormBody = A2($List.map,
      function (position) {
         return A2($Html.div,
         _U.list([snakeStyle(position)]),
         _U.list([$Html.text("😛")]));
      },
      _p7.previousPositions);
      var wormHead = A2($Html.div,
      _U.list([snakeStyle(_p7.position)]),
      _U.list([$Html.text("🐤")]));
      var wormNode = A2($List._op["::"],wormHead,wormBody);
      return A2($Html.div,
      _U.list([containerStyle]),
      A2($List._op["::"],appleNode,wormNode));
   };
   ...
   A2($Signal.map,$Time.inSeconds,$Time.fps(framesPerSecond)));
   var State = F3(function (a,b,c) {
      return {running: a,snake: b,apple: c};
   });
   var Snake = F4(function (a,b,c,d) {
      return {position: a
             ,previousPositions: b
             ,points: c
             ,direction: d};
   ...
   var Apple = F2(function (a,b) {
      return {seed: a,position: b};
   });
   var None = {ctor: "None"};
   var initialState = {running: false
                      ,apple: {seed: $Random.initialSeed(123)
                              ,position: $Maybe.Nothing}
                      ,snake: {position: {x: $Basics.round(mapSize / 2)
                                         ,y: $Basics.round(mapSize / 2)}
                              ,previousPositions: _U.list([])
                              ,points: 0
                              ,direction: None}};
   var stepSnake = F3(function (_p9,_p8,apple) {
      var _p10 = _p9;
      var _p17 = _p10.direction;
      var _p11 = _p8;
      var _p16 = _p11;
      var _p15 = _p11.position;
      var _p14 = _p11.points;
      var newPoints = A2(snakeTouchesApple,
      _p16,
      apple) ? _p14 + 1 : _p14;
      var previousPositions = A2($List._op["::"],
      _p16.position,
      _p16.previousPositions);
      var slicedPreviousPositions = A2($List.take,
      _p16.points,
      previousPositions);
      var newPosition = {x: function () {
                           var _p12 = _p16.direction;
                           switch (_p12.ctor)
                           {case "Left": return _p15.x - 1;
                              case "Right": return _p15.x + 1;
                              default: return _p15.x;}
                        }()
                        ,y: function () {
                           var _p13 = _p16.direction;
                           switch (_p13.ctor)
                           {case "Up": return _p15.y - 1;
                              case "Down": return _p15.y + 1;
                              default: return _p15.y;}
                        }()};
      var cappedPosition = capPosition(newPosition);
      var newDirection = _U.eq(_p17,None) ? _p16.direction : _p17;
      return {position: cappedPosition
             ,previousPositions: slicedPreviousPositions
             ,points: newPoints
             ,direction: newDirection};
   });
   var stepGame = F2(function (_p19,_p18) {
      var _p20 = _p19;
      var _p21 = _p18;
      var _p24 = _p21;
      var _p23 = _p21.snake;
      var _p22 = _p21.apple;
      var updatedSnake = A3(stepSnake,_p20,_p23,_p22);
      var gameOver = snakeTouchesItself(_p23);
      var running = _p24.running || !_U.eq(_p20.direction,None);
      var justStarted = $Basics.not(_p24.running) && running;
      var updatedApple = justStarted ? newApple(_p22) : A2(stepApple,
      _p22,
      _p23);
      return gameOver ? initialState : _U.update(_p24,
      {running: running,apple: updatedApple,snake: updatedSnake});
   });
   var Down = {ctor: "Down"};
   var Up = {ctor: "Up"};
   ...
   var currentDirection = function (keys) {
      var _p26 = keys;
      if (_p26.ctor === "::") {
            if (_p26._1.ctor === "[]") {
                  return toDirection(_p26._0);
               } else {
                  return None;
               }
         } else {
   ...
   var arrowKeys = A2($Signal.map,
   function (_p27) {
      return currentDirection($Set.toList(_p27));
   },
   $Keyboard.keysDown);
   var input = A2($Signal.sampleOn,
   delta,
   A3($Signal.map2,Input,arrowKeys,delta));
   ...
   return _elm.Main.values = {_op: _op
                             ...
                             ,stepSnake: stepSnake
                             ,newApple: newApple
                             ,snakeTouchesApple: snakeTouchesApple
                             ,snakeTouchesItself: snakeTouchesItself
                             ,stepApple: stepApple
                             ,stepGame: stepGame
                             ,delta: delta
                             ,Input: Input
