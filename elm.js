...
Elm.LocalStorage.make = function (_elm) {
   ...
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var remove = function (key) {    return $Native$LocalStorage.remove;};
   ...
   var fromJson = F2(function (decoder,str) {
      var _p0 = A2($Json$Decode.decodeString,decoder,str);
      if (_p0.ctor === "Ok") {
            return $Task.succeed($Maybe.Just(_p0._0));
         } else {
            return $Task.fail(UnexpectedPayload(_p0._0));
         }
   });
   var getJson = F2(function (decoder,key) {
      var decode = function (maybe) {
         var _p1 = maybe;
         if (_p1.ctor === "Just") {
               return A2(fromJson,decoder,_p1._0);
            } else {
               return $Task.succeed($Maybe.Nothing);
            }
      };
      return A2($Task.andThen,get(key),decode);
   });
   var NoStorage = {ctor: "NoStorage"};
   return _elm.LocalStorage.values = {_op: _op,get: get,set: set,remove: remove,getJson: getJson,NoStorage: NoStorage,UnexpectedPayload: UnexpectedPayload};
};
Elm.Main = Elm.Main || {};
Elm.Main.make = function (_elm) {
   ...
   var Remove = {ctor: "Remove"};
   var timeSeed = A2($Signal.map,$Random.initialSeed,A2($Signal.map,$Basics.round,$Time.every($Time.second)));
   var keysDown$ = A2($Signal.map,$List.head,A2($Signal.map,$Set.toList,$Keyboard.keysDown));
   var snakeTouchesItself = function (snake) {
      return A2($List.any,function (position) {    return _U.eq(position,snake.position);},snake.previousPositions);
   };
   var touchesSnake = F2(function (snake,position) {
      return A2($List.any,function (p) {    return _U.eq(p,position);},A2($List._op["::"],snake.position,snake.previousPositions));
   });
   var snakeTouchesApple = F2(function (snake,apple) {
      var entityTouches = function (entity) {
         var _p0 = entity.position;
         if (_p0.ctor === "Just") {
               return A2(touchesSnake,snake,_p0._0);
            } else {
               return false;
            }
      };
      var _p1 = apple;
      switch (_p1.ctor)
      {case "BasicApple": return entityTouches(_p1._0);
         case "BonusApple": return entityTouches(_p1._0);
         default: return entityTouches(_p1._0);}
   });
   var toPixels = function (value) {    return A2($Basics._op["++"],$Basics.toString(value),"px");};
   var toBlockStyle = F2(function (blockSize,position) {
      return $Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "width",_1: toPixels(blockSize)}
                                            ,{ctor: "_Tuple2",_0: "height",_1: toPixels(blockSize)}
                                            ,{ctor: "_Tuple2",_0: "top",_1: toPixels(position.y)}
                                            ,{ctor: "_Tuple2",_0: "left",_1: toPixels(position.x)}
                                            ,{ctor: "_Tuple2",_0: "position",_1: "absolute"}]));
   });
   var containerStyle = F2(function (canvasSize,blockSize) {
      return $Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "width",_1: toPixels(canvasSize)}
                                            ,{ctor: "_Tuple2",_0: "height",_1: toPixels(canvasSize)}
                                            ,{ctor: "_Tuple2",_0: "font-size",_1: toPixels(blockSize)}]));
   });
   var getOverlay = function (points) {
      return _U.eq(points,1) ? $Maybe.Just("👌") : _U.eq(points,2) ? $Maybe.Just("🙌") : _U.eq(points,5) ? $Maybe.Just("👍") : _U.eq(points,
      10) ? $Maybe.Just("😍") : _U.eq(points,20) ? $Maybe.Just("💪") : _U.eq(points,50) ? $Maybe.Just("👏") : _U.eq(points,75) ? $Maybe.Just("🎉") : _U.eq(points,
      100) ? $Maybe.Just("💎") : _U.eq(points,150) ? $Maybe.Just("🌟") : $Maybe.Nothing;
   };
   var highscoreLoaded = $Signal.mailbox(0);
   var getHighscore = Elm.Native.Task.make(_elm).perform(function () {
      var handle = function (str) {
         var _p2 = str;
         if (_p2.ctor === "Just") {
               return A2($Signal.send,highscoreLoaded.address,A2($Result.withDefault,0,$String.toInt(_p2._0)));
            } else {
               return A2($Signal.send,highscoreLoaded.address,0);
            }
      };
      return A2($Task.andThen,$LocalStorage.get("highscore"),handle);
   }());
   var mapSize = 20;
   var view = F3(function (_p4,_p3,highscore) {
      var _p5 = _p4;
      var _p13 = _p5.snake;
      var _p6 = _p3;
      var logoNode = $Basics.not(_p5.logoVisible) ? A2($Html.img,
      _U.list([$Html$Attributes.src("logo.png"),$Html$Attributes.$class("logo logo--hidden")]),
      _U.list([])) : A2($Html.img,_U.list([$Html$Attributes.src("logo.png"),$Html$Attributes.$class("logo")]),_U.list([]));
      var overlayNode = A2($Html.div,
      _U.list([]),
      A2($List.map,
      function (_p7) {
         var _p8 = _p7;
         return A2($Html.div,_U.list([$Html$Attributes.$class("overlay")]),_U.list([$Html.text(_p8._1)]));
      },
      $Array.toList(_p5.overlays)));
      var pointsNode = A2($Html.div,
      _U.list([]),
      _U.list([A2($Html.div,_U.list([$Html$Attributes.$class("points")]),_U.list([$Html.text($Basics.toString(_p13.points))]))
              ,A2($Html.div,_U.list([$Html$Attributes.$class("highscore")]),_U.list([$Html.text($Basics.toString(highscore))]))]));
      var canvasSize = A2($Basics.min,_p6._0,_p6._1);
      var blockSize = $Basics.round($Basics.toFloat(canvasSize) / $Basics.toFloat(mapSize));
      var blockStyle = toBlockStyle(blockSize);
      var scale = function (position) {    return {x: position.x * blockSize,y: position.y * blockSize};};
      var snakeHead = A2($Html.div,_U.list([$Html$Attributes.$class("head"),blockStyle(scale(_p13.position))]),_U.list([$Html.text("😎")]));
      var snakeBody = A2($List.map,
      function (position) {
         return A2($Html.div,_U.list([blockStyle(scale(position))]),_U.list([$Html.text("😂")]));
      },
      _p13.previousPositions);
      var snakeNode = A2($List._op["::"],snakeHead,snakeBody);
      var appleNode = function () {
         var _p9 = _p5.apple;
         switch (_p9.ctor)
         {case "BasicApple": var _p10 = _p9._0.position;
              if (_p10.ctor === "Just") {
                    return A2($Html.div,_U.list([blockStyle(scale(_p10._0))]),_U.list([$Html.text("🍔")]));
                 } else {
                    return A2($Html.div,_U.list([]),_U.list([]));
                 }
            case "ReducerApple": var _p11 = _p9._0.position;
              if (_p11.ctor === "Just") {
                    return A2($Html.div,_U.list([blockStyle(scale(_p11._0))]),_U.list([$Html.text("🍎")]));
                 } else {
                    return A2($Html.div,_U.list([]),_U.list([]));
                 }
            default: var _p12 = _p9._0.position;
              if (_p12.ctor === "Just") {
                    return A2($Html.div,_U.list([blockStyle(scale(_p12._0))]),_U.list([$Html.text("💎")]));
                 } else {
                    return A2($Html.div,_U.list([]),_U.list([]));
                 }}
      }();
      return A2($Html.div,
      _U.list([$Html$Attributes.$class("container"),A2(containerStyle,canvasSize,blockSize)]),
      _U.list([A2($Html.div,_U.list([$Html$Attributes.$class("entities")]),A2($List._op["::"],appleNode,snakeNode)),pointsNode,overlayNode,logoNode]));
   });
   var cap = function (num) {    return _U.cmp(num,0) < 0 ? mapSize + num : _U.cmp(num,mapSize) > -1 ? num - mapSize : num;};
   var capPosition = function (_p14) {    var _p15 = _p14;return {x: cap(_p15.x),y: cap(_p15.y)};};
   var framesPerSecond = 15;
   var tick = A3($Signal.foldp,F2(function (x,y) {    return x + y;}),0,A2($Signal.map,$Basics.always(1),$Time.fps(framesPerSecond)));
   var State = F6(function (a,b,c,d,e,f) {    return {running: a,logoVisible: b,snake: c,apple: d,overlays: e,gameEndedAt: f};});
   var Snake = F6(function (a,b,c,d,e,f) {    return {position: a,previousPositions: b,points: c,size: d,direction: e,lastPointAt: f};});
   var ReducerApple = function (a) {    return {ctor: "ReducerApple",_0: a};};
   var BonusApple = function (a) {    return {ctor: "BonusApple",_0: a};};
   var BasicApple = function (a) {    return {ctor: "BasicApple",_0: a};};
   var newApple = F3(function (apple,snake,seed) {
      newApple: while (true) {
         var _p16 = A2($Random.generate,A2($Random.$float,0,1),seed);
         var bonusProbability = _p16._0;
         var seed$ = _p16._1;
         var _p17 = A2($Random.generate,A2($Random.$int,0,mapSize - 1),seed$);
         var xPosition = _p17._0;
         var seed$$ = _p17._1;
         var _p18 = A2($Random.generate,A2($Random.$int,0,mapSize - 1),seed$$);
         var yPosition = _p18._0;
         var seed$$$ = _p18._1;
         var newPosition = {x: xPosition,y: yPosition};
         if (A2(touchesSnake,snake,newPosition)) {
               var _v11 = apple,_v12 = snake,_v13 = seed$$$;
               apple = _v11;
               snake = _v12;
               seed = _v13;
               continue newApple;
            } else if (_U.cmp(bonusProbability,0.9) > 0) return BonusApple({position: $Maybe.Just(newPosition)}); else if (_U.cmp(bonusProbability,0.75) > 0)
               return ReducerApple({position: $Maybe.Just(newPosition)}); else return BasicApple({position: $Maybe.Just(newPosition)});
      }
   });
   var stepApple = F3(function (apple,snake,input) {    return A2(snakeTouchesApple,snake,apple) ? A3(newApple,apple,snake,input.seed) : apple;});
   var Entity = function (a) {    return {position: a};};
   var Input = F3(function (a,b,c) {    return {direction: a,tick: b,seed: c};});
   var None = {ctor: "None"};
   var initialState = {running: false
                      ,logoVisible: true
                      ,gameEndedAt: 0
                      ,overlays: $Array.fromList(_U.list([]))
                      ,apple: BasicApple({position: $Maybe.Nothing})
                      ,snake: {position: {x: $Basics.round($Basics.toFloat(mapSize) / 3),y: $Basics.round($Basics.toFloat(mapSize) / 3)}
                              ,size: 0
                              ,previousPositions: _U.list([])
                              ,points: 0
                              ,lastPointAt: 0
                              ,direction: None}};
   var last$ = function (buffer) {    return A2($Maybe.withDefault,None,$List.head($List.reverse(buffer)));};
   var handleBuffer = F2(function (action,_p19) {
      var _p20 = _p19;
      var _p22 = _p20._1;
      var _p21 = action;
      if (_p21.ctor === "Remove") {
            return {ctor: "_Tuple2",_0: A2($Maybe.withDefault,None,$List.head(_p22)),_1: A2($Maybe.withDefault,_U.list([]),$List.tail(_p22))};
         } else {
            return {ctor: "_Tuple2",_0: _p20._0,_1: A2($List.take,5,A2($List.append,_p22,_U.list([_p21._0])))};
         }
   });
   var Down = {ctor: "Down"};
   var Up = {ctor: "Up"};
   ...
   var Left = {ctor: "Left"};
   var updateDirection = F2(function (old,$new) {
      return _U.eq($new,None) || (_U.eq(old,Up) && _U.eq($new,Down) || (_U.eq(old,Down) && _U.eq($new,Up) || (_U.eq(old,Left) && _U.eq($new,Right) || _U.eq(old,
      Right) && _U.eq($new,Left)))) ? old : $new;
   });
   var stepSnake = F3(function (_p24,_p23,apple) {
      var _p25 = _p24;
      var _p26 = _p23;
      var _p34 = _p26;
      var _p33 = _p26.size;
      var _p32 = _p26.position;
      var _p31 = _p26.points;
      var sizeReduction = function () {    var _p27 = apple;if (_p27.ctor === "ReducerApple") {    return -4;} else {    return 0;}}();
      var applePoints = function () {    var _p28 = apple;if (_p28.ctor === "BonusApple") {    return 10;} else {    return 1;}}();
      var gotPoint = A2(snakeTouchesApple,_p34,apple);
      var newPoints = gotPoint ? _p31 + applePoints : _p31;
      var newSize = gotPoint ? _p33 + applePoints + sizeReduction : _p33;
      var previousPositions = A2($List._op["::"],_p34.position,_p34.previousPositions);
      var slicedPreviousPositions = A2($List.take,_p34.size,previousPositions);
      var newPosition = {x: function () {
                           var _p29 = _p34.direction;
                           switch (_p29.ctor)
                           {case "Left": return _p32.x - 1;
                              case "Right": return _p32.x + 1;
                              default: return _p32.x;}
                        }()
                        ,y: function () {
                           var _p30 = _p34.direction;
                           switch (_p30.ctor)
                           {case "Up": return _p32.y - 1;
                              case "Down": return _p32.y + 1;
                              default: return _p32.y;}
                        }()};
      var cappedPosition = capPosition(newPosition);
      var newDirection = A2(updateDirection,_p34.direction,_p25.direction);
      return {position: cappedPosition
             ,previousPositions: slicedPreviousPositions
             ,points: newPoints
             ,direction: newDirection
             ,size: A2($Basics.max,0,newSize)
             ,lastPointAt: gotPoint ? _p25.tick : _p34.lastPointAt};
   });
   var stepGame = F2(function (_p36,_p35) {
      var _p37 = _p36;
      var _p48 = _p37.tick;
      var _p47 = _p37;
      var _p38 = _p35;
      var _p46 = _p38;
      var _p45 = _p38.snake;
      var _p44 = _p38.overlays;
      var _p43 = _p38.gameEndedAt;
      var _p42 = _p38.apple;
      var gameHasEnded = !_U.eq(_p43,0);
      var updatedSnake = A3(stepSnake,_p47,_p45,_p42);
      var gameEnded = gameHasEnded || snakeTouchesItself(updatedSnake);
      var updatedGameEndedAt = gameEnded && $Basics.not(gameHasEnded) ? _p48 : _p43;
      var newOverlays = function () {
         if (_U.cmp(updatedSnake.lastPointAt,_p45.lastPointAt) > 0) {
               var _p39 = getOverlay(updatedSnake.points);
               if (_p39.ctor === "Just") {
                     return A2($Array.push,{ctor: "_Tuple2",_0: _p48,_1: _p39._0},_p44);
                  } else {
                     return _p44;
                  }
            } else if (gameEnded) return A2($Array.push,{ctor: "_Tuple2",_0: _p48,_1: "👷"},_p44); else return _p44;
      }();
      var updatedOverlays = A2($Array.filter,function (_p40) {    var _p41 = _p40;return _U.cmp(_p48 - _p41._0,framesPerSecond * 3) < 0;},newOverlays);
      var running = _p46.running || !_U.eq(_p37.direction,None);
      var logoVisible = $Basics.not(running);
      var justStarted = $Basics.not(_p46.running) && running;
      var updatedApple = justStarted ? A3(newApple,_p42,_p45,_p37.seed) : A3(stepApple,_p42,_p45,_p47);
      return gameHasEnded && _U.cmp(_p48 - updatedGameEndedAt,framesPerSecond * 3) > 0 ? initialState : gameHasEnded ? _p46 : _U.update(_p46,
   ...
   var toDirection = function (keyCode) {
      var _p49 = keyCode;
      switch (_p49)
      {case 37: return Left;
         case 38: return Up;
   ...
   var currentDirection = function (key) {    var _p50 = key;if (_p50.ctor === "Just") {    return toDirection(_p50._0);} else {    return None;}};
   var arrowKeys = function () {
      var directions = A3($Signal.filter,function (direction) {    return !_U.eq(direction,None);},None,A2($Signal.map,currentDirection,keysDown$));
      var merged = A2($Signal.merge,A2($Signal.map,$Basics.always(Remove),tick),A2($Signal.map,Add,directions));
      var buffer = A2($Signal.map,$Basics.fst,A3($Signal.foldp,handleBuffer,{ctor: "_Tuple2",_0: None,_1: _U.list([])},merged));
      return buffer;
   }();
   var input = A2($Signal.sampleOn,tick,A4($Signal.map3,Input,arrowKeys,tick,timeSeed));
   var gameState = A3($Signal.foldp,stepGame,initialState,input);
   var currentPoints = A2($Signal.merge,highscoreLoaded.signal,A2($Signal.map,function (state) {    return state.snake.points;},gameState));
   var highscore = A3($Signal.foldp,$Basics.max,0,currentPoints);
   var main = A4($Signal.map3,view,gameState,$Window.dimensions,highscore);
   var saveHighscore = Elm.Native.Task.make(_elm).performSignal("saveHighscore",
   A2($Signal.map,$LocalStorage.set("highscore"),A2($Signal.map,$Basics.toString,highscore)));
